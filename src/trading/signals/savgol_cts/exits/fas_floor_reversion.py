...
from src.trading.signals.savgol_cts.config import FasFloorReversionExitConfig
from src.trading.signals.savgol_cts.state import SavgolCTSExitState


def exit_fas_floor_reversion(
    row: dict,
    prev_row: dict,
    trade: Trade,
    peak_close: float,
    bars_held: int,
    state_val: int,
    cfg: FasFloorReversionExitConfig,
    records: list[dict] | None = None,
    idx: int = 0,
) -> tuple[str | None, int]:
    """Exit condition for FAS Floor Reversion path.
    ...
    if any(np.isnan(x) for x in [cts, prev_cts, cts_st]):
        return None, st.to_int()
