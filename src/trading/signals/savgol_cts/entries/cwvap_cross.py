...
def check_cwvap_cross(
    ...
    # cts_accel must be above threshold and rising
    ...
    if np.isnan(cts_accel) or np.isnan(cts_accel_threshold) or np.isnan(pcts_accel):
    ...
    accel_delta = cts_accel - pcts_accel
    if accel_delta <= cfg.cwvap_cross.accel_delta_min:
        return False, 0, {"reason": f"accel_delta {accel_delta:.5f} <= min {cfg.cwvap_cross.accel_delta_min}"}
