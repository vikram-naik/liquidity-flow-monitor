...
#
# Signature: fn(value: float, params: dict, direction: str) -> float
...
def _higher(value: float, params: dict, direction: str) -> float:
    """Score increases linearly as value rises toward max_value."""
    if math.isnan(value):
        return 0.0
...
def _lower(value: float, params: dict, direction: str) -> float:
    """Score increases as value drops below ref_value toward 0."""
    if math.isnan(value):
        return 0.0
    ref = params.get("ref_value", 1.0)
    if ref == 0:
        return 0.0
    return max(0.0, min(1.0 - value / ref, 1.0))


@register("abs_higher_is_better")
def _abs_higher(value: float, params: dict, direction: str) -> float:
    """Score increases with absolute magnitude (direction-agnostic)."""
    if math.isnan(value):
        return 0.0
    max_val = params.get("max_value", 1.0)
...
def _directional(value: float, params: dict, direction: str) -> float:
    """Score based on whether value aligns with direction.
...
def _count_ratio(value: float, params: dict, direction: str) -> float:
    ...
    return max(0.0, min(value / max_count, 1.0))
