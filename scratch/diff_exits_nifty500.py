...
def main():
    ...
    symbols = get_watchlist_symbols("NIFTY 500")
    test_end = today_str()
    
    # 1. Run with NEW logic (bypass enabled when cts > st)
    print("Running NIFTY 500 simulation with NEW logic (bypass enabled when cts > st)...", flush=True)
    new_trades = run_period(symbols, "2024-01-01", test_end, entry_cfg, exit_cfg, "TEST", signal)
    
    # 2. Run with OLD logic (bypass disabled)
    print("\nRunning NIFTY 500 simulation with OLD logic (bypass disabled)...", flush=True)
    ...
    print("\n=== COMPARISON OF NIFTY 500 CHANGED TRADES ===")
    ...
    print(f"\nTotal changed trades in NIFTY 500: {changed_count}")
