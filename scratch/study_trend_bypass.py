...
BACKUP_PATH = UNIVERSAL_CROSS_PATH.with_suffix(".py.trendbak")
...
def modify_entry_file_with_bypass(cwc_thresh):
    ...
    # Bypasses high-range constraints if cohorts are in a highly coherent trend (cwc > {cwc_thresh})
    ...
    strong_coherent_trend = (cwc > {cwc_thresh})
    
    if in_high_range and not strong_coherent_trend:
    ...
    if target_block in content:
        ...
        with open(UNIVERSAL_CROSS_PATH, "w") as f:
            f.write(new_content)
        print(f"Injected dynamic CWC bypass: cwc > {cwc_thresh}")
    else:
...
def main():
    print("Starting Trend-Coherence Range Bypass Study...")
    ...
    configs = [
        ...
        ("Static Range Filter (No Bypass)", 0.999), # cwc > 0.999 is impossible, effectively no bypass
        ("Dynamic Bypass: cwc > 0.50", 0.50),
        ("Dynamic Bypass: cwc > 0.40", 0.40),
    ]
    
    results = []
    backup_file()
    try:
        for name, cwc_val in configs:
            cache = get_cache()
            cache.clear()
            print("\n------------------------------------------------------------")
            ...
            if cwc_val is None:
                ...
                modify_entry_file_with_bypass(cwc_val)
    ...
    print("\n\n=== RANGE DYNAMIC BYPASS SWEEP COMPLETE ===")
