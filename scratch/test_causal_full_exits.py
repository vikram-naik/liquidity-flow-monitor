...
from scripts.walk_forward import simulate_trades, summarize
...
def main():
    ...
    # We will test higher thresholds: 3.50, 4.00, 4.50, 4.75, 5.00, 5.25
    # Running inside the actual SavgolCTSSignal exits
    thresholds = [3.50, 4.00, 4.50, 4.75, 5.00, 5.25]
    results = []
    
    exit_cfg = SavgolCTSExitConfig()
    # Let's verify standard exits are fully enabled (structural climax, early release, etc. are on in default exit_cfg)
    ...
    for thresh in thresholds:
        ...
        class MockSignalWrapper:
            ...
            def check_entry(self, row, prev_row, cfg, records, idx):
                ...
                if passed:
                    # Return UNIVERSAL_CROSS tag so it routes through universal cross exits
                    return True, 85, {"reason": "OracleMimic Bayesian cross", "entry_tag": EntryTag.UNIVERSAL_CROSS.value, "score": 85}
    ...
    print("\n--- Production Exits Sweep Results ---")
