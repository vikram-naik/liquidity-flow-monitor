"""
PSZ & CTS Slope Zero-Cross Study
================================
Evolving a new entry path: 
1. psz (price_slope_z) crossing zero upward.
2. cts_slope crossing zero upward.
...
def simulate_trades_study(
    ...
    for i in range(10, n):
        ...
        if in_trade:
            ...
            trade.total_slope_delta = sig['details']['cts_slope_total_delta']
            ...
            psz_cross_up = psz_now > 0 and psz_1 <= 0
            cts_slope_cross_up = cts_slope_now > 0 and cts_slope_1 <= 0
            cts_slope_rising = cts_slope_now > cts_slope_1 > cts_slope_2
            v_rising = v_now > v_1 > v_2
            cwc_slope_now = row.get("cwc_slope", 0)
            cwvap_now = row.get("cwvap", 0)
            cwvap_dist = (close - cwvap_now) / cwvap_now * 100 if cwvap_now > 0 else 0
            
            cts_slope_total_delta = cts_slope_now - cts_slope_3
            
            if psz_cross_up and cts_slope_cross_up and cts_slope_rising and v_rising and cwc_slope_now > 0 and cwvap_dist < 0.5 and cts_now > -0.5 and cts_slope_total_delta > 0.05:
                ...
                pending_signal = {
                    "details": {
                        ...
                        "cts_slope_total_delta": cts_slope_total_delta,
...
def main():
    ...
    out = io.StringIO()
    out.write(f"PSZ & CTS Slope Zero-Cross Study\n")
