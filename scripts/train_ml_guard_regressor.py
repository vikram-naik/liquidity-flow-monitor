...
def evaluate(
    ...
    mae = mean_absolute_error(y_actual_pnl, preds_continuous)

    log.info("\n── %s Evaluation (Threshold: Predicted PnL >= %.4f%%) ──", split_name, threshold)
    log.info("MAE (Actual vs Predicted PnL): %.4f%%", mae)
    log.info(
...
def trade_efficiency_report(
    ...
    """
    Bucket all trades by their predicted PnL score and report quality.
    """
    ...
    frame["bucket"] = pd.qcut(
        frame["pred_pnl"].rank(method="first"),
        n_buckets,
    ...
    for b, grp in frame.groupby("bucket", observed=True):
        ...
        row = {
            "bucket":           int(b),
            "score_range":      f"[{grp['pred_pnl'].min():.4f}, {grp['pred_pnl'].max():.4f}]",
            "n_trades":         len(grp),
...
def main() -> None:
    ...
    # ── Target Variable (Continuous PnL) ─────────────────────────────────────
    # We do NOT build binary labels for training. The target is the actual PnL.
    target_col = "pnl_pct"
    ...
    trade_efficiency_report(best_model, X_val,  y_val_reset,  "Validation")
    trade_efficiency_report(best_model, X_test, y_test_reset, "Test")
    ...
    model_path = models_dir / f"model_reg_{date_str}.joblib"
    meta_path  = models_dir / f"model_reg_{date_str}_meta.json"
    fi_path    = models_dir / f"model_reg_{date_str}_feature_importance.csv"
